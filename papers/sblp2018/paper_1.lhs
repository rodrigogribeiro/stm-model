...
over the input string. We developed a prototype implementation in Haskell, verify it using QuickCheck and
...
recent research works (e.g ~\cite{Firsov13,Ribeiro2017,Lopes2018,Lopes2016,Asperti10}).
...
such VM instructions and Lua library LPEG~\cite{Ierusalimschy2009} define a VM whose instruction
...
it does not provide a evidence for matching, which could be used to characterize a disambiguation strategy, like
...
specify, implement and prove the correctness of a VM based algorithm for RE parsing which produces bit-codes as
a memory efficient representation of parse-trees. We leave the task of proving that our VM follows a specific disambiguation
strategy to future work.

Our contributions are:

\begin{itemize}
  \item We present an interpretation of Thompson's NFA\footnote{Non-deterministic finite automata.}
        construction~\cite{Thompson1968} using a small-step operational semantics. The main novelty of this presentation is
  ...
  \item We show how our proposed semantics can produce bit codes that denote parse trees~\cite{Lasse2011} and verify that
...
to model non-determinism~\cite{Lipovaca2011}.  All source code produced, including the literate Haskell source of this
...
\]
where $a$ is any symbol from the underlying alphabet. In our Haskell implementation, we represent alphabet
symbols using type |Char|.
...
parse trees be terms whose type is is underlying RE.
...
\paragraph{Bit-coded parse trees.} Nielsen et. al.~\cite{Nielsen2011} proposed the
...
showed an inductively defined relation between valid bit-codes and RE, accordingly to the coding
proposed by~\cite{Nielsen2011}. We let the judgement $bs \rhd e$ denote that the sequence of bits
...
To define function |decode|, we need to keep track to the remaining bits to be processed to finish
...
The usage of evaluation contexts is standard in operational semantics~\cite{Felleisen2009}.
...
Having defined our semantics, we have noticed that our RE context syntax is exactly the data type
...
\begin{itemize}
  \item $d$ is a direction, which specifies if the semantics is starting (denoted by $S$) or
  ...
  \item $s$ is the input string being currently processed.
\end{itemize}
...
for some expression should end.
...
In all recursive cases for RE, we insert context information in the third component of the resulting configuration.
...
The next theorem asserts that our semantics is sound and complete with respect to RE inductive semantics.

\begin{Theorem}
   For all strings $s$ and REs $e$, $s\in\sembrackets{e}$ if, and only if, $\conf{B,e,[],[],s}\to^\star\conf{E,e',[],b,[]}$ and
   $\conf{E,e',[],b,[]}$ is an accepting configuration.
\end{Theorem}
...
\end{spec}
The remaining equations of |next| deals with operational semantics finishing rules. The equation below implements
...
%format taccept = "\F{accept}"
%format bitcode = "\F{bitcode}"
%format init_cfg = "\F{init_{cfg}}"
%format head = "\F{head}"
%format null = "\F{null}"
%format reverse = "\F{reverse}"
The reflexive-transitive closure of semantics is implemented by function |steps|, which computes the
...
use QuickCheck function |suchThat| which generates a random value with satisfies
...
For simplicity and brevity, we only generate REs that do not contain sub-REs of
the form $e^\star$, where $e$ is nullable\footnote{A RE $e$ is \emph{nullable} if $\epsilon \in \sembrackets{e}$.}.
All results can be extended to non-problematic\footnote{We say that a RE $e$ is problematic if there's $e'$
such that $e = e'^\star$ and $\epsilon \in\sembrackets{e'}$.} REs in the
style of Frisch et. al~\cite{Frisch2004}.
...
As future work, we intend to formalize the our semantics in Coq proof-assistant in order
to extract a correct-by-construction implementation of our algorithm.
...
Nielsen and Henglein~\cite{Nielsen2011} showed how to generate a compact \textit{bit-coded} representation of a parse tree for a
...
An algorithm for POSIX RE parsing is described in~\cite{SulzmannL14}. The main idea of the article is to adapt
...
In this work, we presented an operational semantics for a virtual machine for RE parsing inspired on Thompson's NFA construction.
The key insight of our semantics is to use data-type derivatives to represent evaluation contexts for RE and to produce, as
parsing evidence, bit-codes. We have developed a prototype implementation of our semantics in Haskell and use QuickCheck to
verify its relevant properties with respect to a simple implementation of RE parsing by Fisher et. al.~\cite{Fischer2010}.

As future work we intend to formalize our semantics in Coq proof-assistant~\cite{Bertot2010} and use it to build a certified tool for RE
parsing and investigate how other algorithms (e.g. the Glushkov construction~\cite{Gluskov1961}) for converting a RE into a finite
state machine could be expressed in terms of an operational semantics.

\vspace{-2.0ex}

\section*{Acknowledgements}

We would like to thank Prof. \emph{Leonardo Vieira}  and \emph{Samuel Feitosa} for their valuable suggestions and comments on early

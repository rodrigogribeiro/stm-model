...
provide support for high-level operators like |orElse| and |retry|.
...
consider the following function that increments an value stored on a |TVar|,
...
the value of a variable onto the stack and |Write| updates the variable with topmost value in the VM stack. Instructions |Begin| and |Commit|
...
Type |Offset| is just a synonym for |Int| and denotes how much should we move to execute the next instruction. Data type
...
during VM execution. We allow ourselves a bit of informality and represent values of type |StkCell| without explicitly mark them with
...
Instructions |Push| and |Add| modify the stack by putting a new value on top of the stack and poping two values and
...
|Read| pushes the value of variable v on top of the stack. First it consults the write log.
...
Semantics of |Jump| and |JumpFalse| only differ in that the latter jumps only if the topmost element of current
...
\[
  ...
  \end{array}
\]
Instruction |Mark| pushs handler code and |Unmark| removes a handler from current thread local stack.
\[
   \begin{array}{c}
      \infer[_{(Mark)}]
            {\langle \Theta_h,\,\sigma,\,j,t : s\rangle \mapsto_{VM}
...
To implement such behavior we use functions |handler|, which searches for a handler on thread's stack and |skip|, which
removes remaining transaction instructions from threads code, by stopping when it finds a |Unmark|.
\begin{spec}
